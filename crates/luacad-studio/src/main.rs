...
fn main() {
  ...
  unsafe {
    ...
    let vendor = c_str(glGetString(GL_VENDOR));
    let renderer = c_str(glGetString(GL_RENDERER));
    let version = c_str(glGetString(GL_VERSION));
    let extensions = c_str(glGetString(GL_EXTENSIONS));
    eprintln!("[GL] Vendor:   {vendor}");
    eprintln!("[GL] Renderer: {renderer}");
    eprintln!("[GL] Version:  {version}");
    ...
    let has_ext_pds = extensions.contains("GL_EXT_packed_depth_stencil");
    eprintln!("[GL] GL_ARB_framebuffer_object:  {has_arb_fbo}");
    eprintln!("[GL] GL_EXT_framebuffer_object:  {has_ext_fbo}");
    eprintln!("[GL] GL_EXT_packed_depth_stencil: {has_ext_pds}");
    if !has_arb_fbo && !(has_ext_fbo && has_ext_pds) {
      eprintln!(
        "[GL] WARNING: No framebuffer object support detected — OpenCSG models will not render"
      );
    }
  }
